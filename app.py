...
ACCOUNTS = [] # Danh sách tài khoản load từ file
        ...
        try:
            ...
            with active_spam_lock:
                if target_id not in active_spam_targets:
                    ...
                    # Chạy thread spam riêng biệt
                    threading.Thread(target=spam_worker, args=(target_id, duration_minutes), daemon=True).start()
                    
                    message = f"Bắt đầu spam UID: {target_id}"
                    if duration_minutes:
                        message += f" trong {duration_minutes} phút"
                    return {"status": "success", "message": message}
                else:
                    return {"status": "error", "message": "UID này đang chạy spam rồi"}
        ...
        try:
            with active_spam_lock:
                if target_id in active_spam_targets:
                    ...
                    return {"status": "success", "message": f"Đã dừng spam UID: {target_id}"}
                else:
                    return {"status": "error", "message": "UID này không có trong danh sách spam"}
        except Exception as e:
...
def spam_worker(target_id, duration_minutes=None):
    print(f"[API] Bắt đầu luồng spam cho {target_id}")
    start_time = datetime.now()
    
    while True:
        with active_spam_lock:
            if target_id not in active_spam_targets:
                print(f"[API] Dừng luồng spam cho {target_id}")
                break
            
            if duration_minutes:
                elapsed = (datetime.now() - start_time).total_seconds()
                if elapsed >= duration_minutes * 60:
                    print(f"[API] Hết thời gian spam cho {target_id}")
                    del active_spam_targets[target_id]
                    break
                
        try:
            send_spam_from_all_accounts(target_id)
            time.sleep(0.5) # Nghỉ nhẹ để tránh treo CPU Render
        except Exception as e:
...
def run_api():
    # Cấu hình PORT cho Render (mặc định 8080 nếu chạy local)
    port = int(os.environ.get("PORT", 8080))
    print(f"🌐 API đang chạy trên Port: {port}")
...
def load_accounts_from_file(filename="accs.txt"):
    ...
    try:
        ...
        print(f"✅ Đã load {len(loaded)} tài khoản từ {filename}")
    except Exception as e:
        print(f"❌ Lỗi đọc file accs.txt: {e}")
    return loaded
...
class FF_CLient():
    ...
    def Connect_SerVer(self , Token , tok , host , port , key , iv , host2 , port2):
        try:
            ...
            with connected_clients_lock:
                ...
                print(f" [+] Tài khoản {self.id} đã online!")
            
            while True:      
                # Chỉ nhận dữ liệu để duy trì kết nối
                # ĐÃ XÓA: Phần check '/pp/' để chặn auto spam
                self.DaTa = self.CliEnts.recv(1024)   
                if len(self.DaTa) == 0:
                    raise Exception("Disconnect")
                    
        except Exception as e:
            # print(f"Mất kết nối {self.id}: {e}")
            try:
    ...
    def ToKen_GeneRaTe(self , Access_ToKen , Access_Uid):
        ...
        # --- RAW HEX DATA (GIỮ NGUYÊN KHÔNG CẮT BỚT) ---
        ...
        # Cập nhật thời gian và token vào payload
...
def StarT_SerVer():
    ...
    # 2. Load accounts từ file
    global ACCOUNTS
    ACCOUNTS = load_accounts_from_file("accs.txt")
    
    if not ACCOUNTS:
        print("!!! CẢNH BÁO: Chưa có tài khoản nào được load. Hãy kiểm tra file accs.txt !!!")
    
    # 3. Khởi chạy từng account
    threads = []
    for account in ACCOUNTS:
        ...
        thread.start()
        time.sleep(3)  # Delay giữa các lần login để tránh bị chặn IP
